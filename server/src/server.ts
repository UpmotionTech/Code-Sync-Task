...
io.on("connection", (socket) => {
	...
	socket.on(SocketEvent.JOIN_REQUEST, ({ roomId, username }) => {
		...
		// First user to join the room becomes the admin
		const isFirstInRoom = getUsersInRoom(roomId).length === 0
		const user = {
			username,
			roomId,
	...
	socket.on(SocketEvent.BROADCAST_MESSAGE, ({ message }) => {
		const sender = getUserBySocketId(socket.id)
		if (!sender) return
		if (!sender.isAdmin) {
			console.warn(
				`[BROADCAST] Rejected: ${sender.username} is not an admin`,
			)
			return
		}
		const roomId = sender.roomId
		// Emit to ALL clients in the room including the sender
		io.to(roomId).emit(SocketEvent.RECEIVE_BROADCAST, { message })
		console.log(
			`[BROADCAST] ${sender.username} in room ${roomId}: "${message.message}"`,
		)
	})
